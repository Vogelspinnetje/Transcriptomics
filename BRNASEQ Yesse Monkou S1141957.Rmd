...
hebben alle samples dus dezelfde type borstkanker. Er zijn vier samples
genomen voor behandeling een vier samples na behandeling.

## Experimenteel Design

Zoals getoond zitten de onafhankelijke variabelen in metadata9.txt. Het gaat hier om de kolom voor.na die als enige verschil toont. Daarom wordt dit ook de conditie in het experimenteel design.
...
```{r echo=FALSE}
boxplot(log2(countdata + 1), las=2,
        main="Boxplot van genexpressie per sample",
        ylab="log2(counts + 1)")

log_counts <- log2(countdata + 1)

colors <- rainbow(ncol(log_counts))

plot(density(log_counts[,1]), col=colors[1], lwd=2,
     main="Density plots van genexpressie per sample",
     xlab="log2(counts + 1)", ylim=c(0, 0.25))

for(i in 2:ncol(log_counts)){
  lines(density(log_counts[,i]), col=colors[i], lwd=2)
}

legend("topright", legend=colnames(log_counts), col=colors, lwd=2, cex=0.7)
```
...
kunnen hoge waardes interessant zijn, maar outliers worden niet getolereerd.
Voor deze dataset wordt geconcludeerd dat DESeq voldoende is om de outliers te 
bestrijden.
...
En dan zien de counts er zo uit:

```{r echo=FALSE}
boxplot(log2(normalized_counts + 1), las=2,
        main="Boxplot van genormaliseerde genexpressie per sample",
        ylab="log2(counts + 1)")
...
plot(density(log_counts[,1]), col=colors[1], lwd=2,
     main="Density plots van genormaliseerde genexpressie per sample",
     xlab="log2(counts + 1)", ylim=c(0, 0.25))
...
deze samples te behouden. Wat voor de rest natuurlijk op valt, is dat de "voor" 
en "na" data allebei in hetzelfde cluster zitten (op die twee eerder genoemde
datapunten na).
